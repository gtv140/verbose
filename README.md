...
// CREATE PLANS 1..25 (200 - 30000), 7 special offers, last 5 coming soon  
for(let i=1;i<=25;i++){  
    const invest = Math.round(200 + (i-1)*(30000-200)/24);  
    const days = 20 + Math.floor((i-1)*(70-20)/24);  
    const multiplier = i<=7 ? 3 : 2.5;  
    const comingSoon = i > 20; // last 5 are coming soon (21-25)  
    plansData.push({  
      id:i,  
      name:`Plan ${i}`,  
      invest:invest,  
...
function addDailyProfit(){
    ...
    const lastDate = localStorage.getItem(`verbose_dailyDate_${currentUser}`);
    if(lastDate === today) return; // already added today
    let totalDaily = 0;
    userPlans.forEach(p=>{
        const plan = plansData.find(x=>x.id===p.planId);
        if(plan){
            totalDaily += Math.round(plan.total/plan.days);
        }
    });
    dailyProfit = (parseFloat(localStorage.getItem('verbose_daily')) || 0) + totalDaily;
    localStorage.setItem('verbose_daily', String(dailyProfit));
    localStorage.setItem(`verbose_dailyDate_${currentUser}`, today);
    document.getElementById('dashDaily').innerText = dailyProfit;
}

// run daily profit every page load
if(currentUser) addDailyProfit();

// === INITIALIZE DEPOSIT NUMBER ===
updateDepositNumber();

// === SUPPORT BOX WHATSAPP NUMBER & GROUP ===
document.querySelectorAll('.support-item').forEach(item=>{
    if(item.querySelector('strong').innerText.includes('WhatsApp Support')){
        item.querySelector('p').innerHTML += `<br>📱 Direct: <a href="https://wa.me/92337982782" target="_blank">03379827882</a>`;
    }
});
</script>  
