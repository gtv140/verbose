    ...
    <div class="support-grid">
      <div class="support-item">
        ...
        <div>
          ...
          <p class="small">Fastest support — join our group or message the admin directly.<br>📱 Direct: <a href="https://wa.me/923705519562" target="_blank">03705519562</a></p>
...
function login(){
    ...
    renderPlans();
    addDailyProfit(); // add daily profit immediately
}
...
function renderPlans(){
    ...
    plansData.forEach(p=>{
        ...
        div.innerHTML=`
            <div class="meta">
                <b>${p.name} ${p.offer ? '<span class="offer">🔥</span>':''}</b>
...
function buyPlan(id){
    ...
    userPlans.push({id:plan.id, start:new Date().toISOString()});
    localStorage.setItem('verbose_userPlans',JSON.stringify(userPlans));
    document.getElementById('dashBalance').innerText=balance;
...
function addDailyProfit(){
    ...
    }, 1000*60*60*24); // 24hr cycle, can reduce to 1min for testing
}
...
function submitDeposit(){
    ...
    if(!tx||!file){alert("Provide TX ID and proof"); return;}
...
function submitWithdraw(){
    ...
    if(!acc||!amt||amt>balance){alert("Invalid withdrawal"); return;}
    balance -= amt;
    localStorage.setItem('verbose_balance',balance);
    document.getElementById('dashBalance').innerText=balance.toFixed(2);
    alert(`Withdrawal request of Rs ${amt} submitted via ${method}`);
}
