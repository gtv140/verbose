...
Invest: Rs ${plan.invest}<br>Total: Rs ${plan.total}<br>Days: ${plan.days}<br>${plan.offer?'<span class="offer">Special Offer! <span class="countdown" id="countdown'+plan.id+'">—</span></span>':''}</div>
<div class="actions">${plan.coming?'Coming Soon':''}</div>`;
