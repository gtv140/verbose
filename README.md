...
function renderTransactions(){
  ...
  allTx.forEach(tx=>{
    ...
    tr.innerHTML=`
      <td>${tx.type || 'Deposit/Withdraw'}</td>
      <td>${tx.amount || 0}</td>
      <td>${tx.plan || '-'}</td>
      <td>${tx.days || '-'}</td>
...
function renderProfile(){
  ...
  stats.innerHTML=`Balance: ${cur.balance || 0} PKR | Daily Profit: ${cur.profit || 0} PKR`;
}
function updateProfile(){
  ...
  document.getElementById('dashUser').innerText=newName;
