...
<div class="wrap">  <!-- LOGIN -->  <div id="loginCard" class="card">  
...
</div>  <!-- DASHBOARD -->  <div id="dashboardCard" class="card hidden">  
<div class="alert-note">⚠️ Notice: All transactions are secure. For any deposit/withdrawal issues, contact Administration immediately. 24/7 support available.</div>  
<div class="user-box">  
<div>  
...
</div>  <!-- ADMIN / SUPPORT / CONTACT -->  <div class="admin-box">  
...
</div>  <!-- PLANS -->  <div id="plansCard" class="card hidden">  
...
</div>  <!-- DEPOSIT -->  <div id="depositCard" class="card hidden">  
...
</div>  <!-- WITHDRAW -->  <div id="withdrawCard" class="card hidden">  
...
const deposits=JSON.parse(localStorage.getItem(KEY_DEPOS<script>  
// CONTINUATION OF DEPOSIT  
const deposits=JSON.parse(localStorage.getItem(KEY_DEPOSITS)||'[]');  
deposits.push({user:currentUser,amount,method:document.getElementById('depositMethod').value,tx:tx,proof:proof.name,date:new Date().toISOString()});  
localStorage.setItem(KEY_DEPOSITS,JSON.stringify(deposits));  
document.getElementById('depositTx').value='';document.getElementById('depositProof').value='';renderDashboard();}  
...
withdraws.push({user:username,amount,method,account,date:new Date().toISOString()});  
localStorage.setItem(KEY_WITHDRAWS,JSON.stringify(withdraws));  
alert(`Withdrawal request submitted!\\nUser: ${username}\\nAmount: Rs ${amount}\\nMethod: ${method}\\nAccount: ${account}\\nContact Admin if any issue.`);  
document.getElementById('withdrawAmount').value='';document.getElementById('withdrawAccount').value='';renderDashboard();}  

// DAILY CREDIT (simulate daily profit for all user plans)  
function computeDailyCredits(){if(!currentUser)return;  
let userPlans=JSON.parse(localStorage.getItem(KEY_USER_PLANS+currentUser)||'[]');  
let bal=Number(localStorage.getItem(KEY_BAL+currentUser)||0);  
let daily=Number(localStorage.getItem(KEY_DAILY+currentUser)||0);  
const now=Date.now();  
userPlans.forEach(up=>{const last=up.lastCredit||0;const daysPassed=Math.floor((now-last)/86400000);  
if(daysPassed>0){bal+=up.dailyProfit*daysPassed;daily+=up.dailyProfit*daysPassed;up.lastCredit=now;}});  
localStorage.setItem(KEY_USER_PLANS+currentUser,JSON.stringify(userPlans));  
localStorage.setItem(KEY_BAL+currentUser,bal);localStorage.setItem(KEY_DAILY+currentUser,daily);  
renderDashboard();}  

// AUTO DAILY CREDIT EVERY MINUTE (simulate daily pass)  
