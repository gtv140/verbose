...
// CREATE PLANS 1..25 (200 - 30000), first 7 special offers, last 5 coming soon
for(let i=1;i<=25;i++){
...
function renderPlans(){
    ...
    list.innerHTML = '';
    plansData.forEach(p => {
        const div = document.create// === RENDER PLANS CONTINUED ===
function renderPlans(){
    const list = document.getElementById('plansList');
    list.innerHTML = '';
    plansData.forEach(p => {
        ...
        div.innerHTML = `
            <div class="meta">
                <b>${p.name} ${p.coming ? '(Coming Soon)' : ''}</b>
                <div class="small">Invest: Rs ${p.invest} | Duration: ${p.days} days</div>
                <div class="small">Total Return: Rs ${p.total} (${p.multiplier}x)</div>
                ${p.offer ? '<div class="offer">Special Offer!</div>' : ''}
            </div>
            <div class="actions">
                ${!p.coming ? `<button onclick="investPlan(${p.id})">Invest</button>` : ''}
            </div>
        `;
...
function investPlan(planId){
    ...
    if(confirm(`Invest Rs ${plan.invest} in ${plan.name}?`)){
        userPlans.push(plan);
        localStorage.setItem('verbose_userPlans', JSON.stringify(userPlans));
...
// === DEPOSIT NUMBER UPDATE ===
function updateDepositNumber(){
    const method = document.getElementById('depositMethod').value;
    let number = '';
    if(method==='jazzcash') number = '03705519562';
    else if(method==='easypaisa') number = '03379827882';
    document.getElementById('depositNumber').value = number;
    document.getElementById('depositAmount').value = '0'; // default
}
...
function submitDeposit(){
    ...
    if(!txId || !file){ alert('Please provide TX ID and upload proof'); return; }
...
function submitWithdraw(){
    ...
    const username = document.getElementById('withdrawUsername').value || currentUser;
    const account = document.getElementById('withdrawAccount').value.trim();
    const amount = parseFloat(document.getElementById('withdrawAmount').value) || 0;
    if(!account || !amount){ alert('Enter account number and amount'); return; }
